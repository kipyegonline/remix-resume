...
export default function SideBar({ setModal, darkMode }: Props) {
  return (
    <aside
      className={`w-full md:w-80 relative md:fixed h-auto md:h-full overflow-y-auto p-6 ${darkMode ? "bg-gray-800" : "bg-gray-100"}`}
    >
      <div className="flex flex-col items-center">
        ...
        <p className="text-sm mb-4">
          ...
          <button
            onClick={() => {
              window.open(location.origin + "?pr", "_blank");
            }}
...
const MySocials = () => {
  ...
  return (
    <div className="flex gap-4 mb-6">
      <a
        href="https://github.com/kipyegonline"
        target="_blank"
        rel="noreferrer"
      ...
      <a
        href="https://www.linkedin.com/in/kipyegonline"
        target="_blank"
        rel="noreferrer"
        className="hover:text-blue-600"
      >
    ...
    <div className="mb-6">
      ...
      <ul className="space-y-2 text-sm">
        <li>
          <button
            onClick={() => {
              window.open(location.search + "?pr", "_target");
            }}
          >
            Public relations{" "}
            <ExternalLink
              size={size}
              className=" cursor-pointer inline ml-2 hover:text-blue-700 transition-colors duration-150 ease"
            />{" "}
          </button>
        </li>
        <li>
